...
const ConnectionStatus = () => {
  ...
  // Track connection state changes để detect reconnect
  useEffect(() => {
  ...
  // Update disconnected duration mỗi giây
  useEffect(() => {
  ...
  // Không hiển thị gì nếu chưa biết trạng thái (initial load)
  if (isConnected === null) {
    return null;
  }

  // Hiển thị banner "reconnected" (success)
  ...
  // Hiển thị banner "disconnected" (danger)
  if (!isConnected) {
    return (
      <div
  ...
  // Connected và không cần hiển thị gì
  return null;
};
