...
class DetectionService:
    ...
    def __init__(self, camera_manager, websocket_manager, ocr_service=None, plate_callback=None):
        self.camera_manager = camera_manager
        self.websocket_manager = websocket_manager
        self.ocr_service = ocr_service
        self.plate_callback = plate_callback  # Callback để lưu OCR result
        ...
        while self.running:
            try:
                    ...
                    if config.ENABLE_OCR and self.ocr_service and self.ocr_service.is_ready() and frame is not None:
                        ...
                        if should_run_ocr:
                            try:
                                ...
                                # DEBUG: Log bbox và frame size
                                print(f"🔍 DEBUG OCR - Frame: {frame_w}x{frame_h}, BBox: x={x}, y={y}, w={w}, h={h}")
                                ...
                                print(f"🔍 DEBUG OCR - Valid BBox: x={x_valid}, y={y_valid}, w={w_valid}, h={h_valid}")

                                # Crop detection region
                                if w_valid > 10 and h_valid > 10:  # Chỉ crop nếu đủ lớn
                                    crop = frame[y_valid:y_valid+h_valid, x_valid:x_valid+w_valid]

                                    print(f"🔍 DEBUG OCR - Crop size: {crop.shape}")

                                    # RESIZE crop lên kích thước tối ưu cho OCR
                                    # OCR models thường hoạt động tốt với height ~200-300px
                                    target_height = 200
                                    if h_valid < target_height:
                                        ...
                                        print(f"🔍 DEBUG OCR - Resized to: {crop.shape}")
                                    ...
                                    if crop.size > 0:
                                        ...
                                        if text:
                                            ...
                                            # Callback để lưu vào app.py
                                            if self.plate_callback:
                                                self.plate_callback(text, confidence, [x, y, w, h], timestamp)
                                else:
                                    print(f"⚠️  BBox too small: w={w_valid}, h={h_valid}")
                ...
                if len(detection_results) > 0:
                    ...
                    # CHỈ broadcast khi CÓ detections (tối ưu bandwidth)
        ...
        try:
            ...
            for box, score, category in zip(boxes, scores, classes):
                if score > config.DETECTION_THRESHOLD:
                    ...
                    if detection_count == 0:  # Chỉ log detection đầu tiên
                        print(f"🔍 DEBUG BBOX - Raw box coords: {box}")
                    ...
                    # DEBUG: Log converted bbox
                    if detection_count == 0:
                        print(f"🔍 DEBUG BBOX - Converted to pixels: x={x}, y={y}, w={w}, h={h}")
                        print(f"🔍 DEBUG BBOX - Frame size should be: {config.RESOLUTION_WIDTH}x{config.RESOLUTION_HEIGHT}")
