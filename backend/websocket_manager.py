...
class WebSocketManager:
    ...
    def broadcast_plate_state(self, plate_state):
        """
        Broadcast plate state (barrier open/close) tới tất cả clients
        Called from sync thread - use asyncio.run_coroutine_threadsafe

        Args:
            plate_state: dict với barrier_open, text, status, time, message
        """
        if not self.active_connections:
            return
